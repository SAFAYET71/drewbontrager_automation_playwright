...
test("Test 23 - Director Dashboard - Creating Camp", async({page}) =>{
  ...
  //Location edit
  await page.getByRole('textbox', { name: 'e.g., Los Angeles, CA or search gym/stadium...' }).click();
  await page.keyboard.press('Control+A')
  await page.keyboard.press('Backspace')
  await page.getByRole('textbox', { name: 'e.g., Los Angeles, CA or search gym/stadium...' }).fill("Barishal Bangladesh");
  // await page.waitForSelector('.pac-item');
  await page.locator('.pac-item', {hasText: 'BarishalBangladesh'}).click();
  //Timezone
  await page.locator('#root').getByText('Bangladesh (BST)').click();
  await page.keyboard.type('Dubai');
  await page.getByText('Dubai (GST)').click();
  //Start Date & End Date
  ...
  //Sport Type 
  await page.locator('#root').getByText('Cricket').click();
  ...
  //Update Camp Button
  await page.locator('button:has-text("Update Camp")').click();
  await expect(page).toHaveURL('https://test.whistleworks.org/director-dashboard');
  //Delete Camp
  await page.getByRole('heading', { name: 'Feb Camp 2'}).click();
  await page.getByRole('button', { name: 'Delete' }).click();
  //Camp Delete Pop-Up
  await expect(page.getByRole('dialog')).toBeVisible();
  await expect(page.getByText('!', { exact: true })).toBeVisible();
  await expect(page.getByRole('heading', { name: 'Are you sure?' })).toBeVisible();
  await expect(page.getByText('This camp will be permanently deleted!', { exact: true })).toBeVisible();
